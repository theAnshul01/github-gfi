...
const IssueCard = ({ path, title, avatar, author, created }: IssueCardProps) => {
    return (
        <a href={path} target="_blank">
            <div className="w-full max-w-96 min-h-80 h-auto border hover:border-green-500 border-[#783434] p-8 font-mono">
                <section className="mb-2">
                    <p className="italic text-green-700 text-xs mb-6 whitespace-nowrap truncate overflow-hidden">{`path: ${path}`}</p>
                    <h1 className="text-gray-200 font-space text-3xl whitespace-normal">{title}</h1>
                </section>
